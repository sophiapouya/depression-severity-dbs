"""
DBS PCA regression LOO CV — pooled scatter plot, patients in different colors.
PCA retains components explaining 90% of variance; OLS regression on PC scores.
Run from project root: python paper_figures/pca_regression_dbs.py
"""
import os
import sys
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pca_regression_dbs")
...
for color, r in zip(colors, results):
    ax.scatter(r["measured"], r["predicted"],
               color=color, s=18, alpha=0.9, linewidths=0, label=r["patient_id"].replace("DBSTRD","TRD"))
...
ax.set_xlabel("Measured Depression Severity", fontsize=6)
ax.set_ylabel("Predicted Depression Severity", fontsize=6)
ax.legend(loc="lower right", fontsize=5, frameon=False)
ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)

out_path = os.path.join(OUT_DIR, "pca_regression_dbs_all_patients.png")
