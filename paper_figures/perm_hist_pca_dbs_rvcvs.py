"""
Permutation histogram for PCA-OLS DBS RVCVS-only results.
Pools all 1000-iteration permutation NRMSE values from
permutation_results/pca_dbs_rvcvs_only/RVCVS/ into one histogram,
then marks each patient's true NRMSE as a vertical line with p-value.
Bold labels indicate p < 0.05 (per-patient permutation test).

Run from project root: python paper_figures/perm_hist_pca_dbs_rvcvs.py
"""
import os
import sys
...
PERM_DIR = os.path.join(
    ...
    "permutation_results", "pca_dbs_rvcvs_only", "RVCVS",
)

OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "perm_hist_pca_dbs_rvcvs")
...
for subj in ALL_SUBJS:
    nrmse_csv = os.path.join(PERM_DIR, f"{subj}_OLS_LOO_RVCVS_permutation_nrmse.csv")
    summary_csv = os.path.join(PERM_DIR, f"{subj}_OLS_LOO_RVCVS_permutation_summary.csv")
...
out_path = os.path.join(OUT_DIR, "perm_hist_pca_dbs_rvcvs.png")
fig.savefig(out_path, dpi=300, bbox_inches="tight")
print(f"Saved: {out_path}")
svg_path = os.path.join(OUT_DIR, "perm_hist_pca_dbs_rvcvs.svg")
