"""
DBS PCA regression LOO CV (RVCVS contacts only) — pooled scatter plot,
patients in different colors.
PCA retains components explaining 90% of variance; OLS regression on PC scores.
Run from project root: python paper_figures/pca_regression_dbs_rvcvs.py
"""
import os
import sys
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pca_regression_dbs_rvcvs")
...
# ── load and filter to RVCVS only ────────────────────────────────────────────
all_df = pd.read_csv(str(FEATURES_DBS))
all_df = all_df.drop(columns=["time"], errors="ignore")

id_cols = [c for c in NON_FEATURE_COLS if c in all_df.columns]
rvcvs_cols = [c for c in all_df.columns if "RVCVS" in c]
all_df = all_df[id_cols + rvcvs_cols]
...
out_path = os.path.join(OUT_DIR, "pca_regression_dbs_rvcvs.png")
fig.savefig(out_path, dpi=300, bbox_inches="tight")
print(f"Saved: {out_path}")
svg_path = os.path.join(OUT_DIR, "pca_regression_dbs_rvcvs.svg")
