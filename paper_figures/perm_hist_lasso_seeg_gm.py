"""
Permutation histogram for Lasso SEEG results (greymatter filtered).
Reads chunk files from permutation_results/lasso_seeg_gm/, merges on the fly,
pools all 1000-iteration permutation NRMSEs into one histogram, then marks
each patient's true NRMSE as a vertical line with p-value.
Bold labels indicate p < 0.05 (per-patient permutation test).

NOTE: This script reads directly from chunk files. All chunks should be present
in permutation_results/lasso_seeg_gm/ (created by seeg_perm_jobs_gm/).

Run from project root: python paper_figures/perm_hist_lasso_seeg_gm.py
"""
import os
import sys
...
PERM_DIR = os.path.join(
    ...
    "permutation_results", "lasso_seeg_gm",
)

OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "perm_hist_lasso_seeg_gm")
...
print(f"\nPooled: {len(pooled_nrmse)} values across {len(ALL_SUBJS)} patients")
...
leg = ax.legend(loc="upper left", bbox_to_anchor=(1.02, 1.0),
                borderaxespad=0, fontsize=5, frameon=False)
...
out_path = os.path.join(OUT_DIR, "perm_hist_lasso_seeg_gm.png")
fig.savefig(out_path, dpi=300, bbox_inches="tight")
print(f"Saved: {out_path}")
svg_path = os.path.join(OUT_DIR, "perm_hist_lasso_seeg_gm.svg")
