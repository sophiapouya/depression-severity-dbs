"""
SEEG PCA permutation test (greymatter filtered, all regions) — chunk-based for SLURM array jobs.
...
if chunk_id == 0:
    ...
    true_df.to_csv(
        os.path.join(chunks_dir, f"{patient_id}_true_decoder.csv"),
        index=False
    )
...
def run_one_permutation(perm_idx):
    ...
    true_scores = patient_df_raw["catdi_score"].to_numpy()
