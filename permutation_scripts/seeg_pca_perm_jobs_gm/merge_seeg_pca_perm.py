"""
Merge chunk outputs into final permutation result files (SEEG PCA greymatter, all regions).

Expects chunks at:
  {BASE_DIR_SEEG}/seeg_pca_permutation_test_gm/chunks/
    {patient_id}_true_decoder.csv
    {patient_id}_chunk000.csv ... chunk{n_chunks-1}.csv

Writes final results to:
  {BASE_DIR_SEEG}/seeg_pca_permutation_test_gm/
    {patient_id}_seeg_pca_permutation_nrmse.csv
    {patient_id}_seeg_pca_permutation_summary.csv

Run from project root:
  python seeg_pca_perm_jobs_gm/merge_seeg_pca_perm.py --patient_id DBSTRD001 --n_chunks 20
"""
import argparse
import os
import sys
...
chunks_dir = os.path.join(base_dir, "seeg_pca_permutation_test_gm", "chunks")
out_dir = os.path.join(base_dir, "seeg_pca_permutation_test_gm")
...
# ── write nrmse CSV ───────────────────────────────────────────────────────────
rows = [
    {"patient_id": patient_id, "type": "permuted",
     "iteration": int(row["iteration"]), "nrmse": row["nrmse"]}
    for _, row in all_chunks.iterrows()
]
rows.append({"patient_id": patient_id, "type": "true",
             "iteration": np.nan, "nrmse": true_nrmse})
nrmse_df = pd.DataFrame(rows)
nrmse_csv = os.path.join(out_dir, f"{patient_id}_seeg_pca_permutation_nrmse.csv")
...
summary_csv = os.path.join(out_dir, f"{patient_id}_seeg_pca_permutation_summary.csv")
