...
def format_timeorigin(header_list):
    ...
    _ = next(header_list) # Day of week (unused)
    ...
    # --- CRITICAL ROBUST DATE FIX ---
    
    # 1. Check Year (min 1, but 0 is common corrupt value)
    if year < 1 or year > 9999: # 9999 is a safe upper limit
        print(f"Warning: Invalid year ({year}) in file header. Substituting year 2000.")
        year = 2000
    
    # 2. Check Month (must be 1-12)
    if not 1 <= month <= 12:
        print(f"Warning: Invalid month ({month}) in file header. Substituting month 1 (Jan).")
        month = 1
        
    # 3. Check Day (must be 1-31, but depends on month/year. We use max 31 and rely on datetime check)
    if not 1 <= day <= 31:
        print(f"Warning: Invalid day ({day}) in file header. Substituting day 1.")
        day = 1
        
    # 4. Final attempt to create datetime object (may still fail if day/month combination is impossible)
    try:
        dt_obj = datetime(year, month, day, hour, minute, second, millisecond * 1000)
    except ValueError as e:
        # Catch issues like Feb 30th after substitution
        print(f"Error: Final date substitution failed to create datetime object. {e}")
        # Fallback to the absolute safest date: 2000-01-01 00:00:00
        dt_obj = datetime(2000, 1, 1, 0, 0, 0, 0)
